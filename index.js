...
async function confirmW(item) {
  ...
  // FormData multipart — format exact my-managment (vérifié par capture réseau)
  ...
  fd.append('report_id'   , '');
  // Headers: Cookie + boundary FormData (PAS de Content-Type manuel)
  const h = {
    'Cookie'    : parseCookies(cfg.mgmtCookies),
    'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36',
    'Referer'   : 'https://my-managment.com/fr/admin/report/pendingrequestwithdrawal',
...
.wa span:last-child{color:var(--o)}.dt span:last-child{color:var(--m)}.in span:last-child{color:var(--b)}
...
<textarea name="mgmtCookies" rows="3" placeholder="PHPSESSID=...; auid=...&#10;ou JSON Firefox">${cfg.mgmtCookies?'(configuré — coller pour remplacer)':''}</textarea>
...
app.post('/save-accounts',(req,res)=>{
  ...
  addLog('ok',`Comptes mis à jour — ${p.split(';').length} cookie(s)`);
