...
const MGMT_COOKIES    = process.env.MGMT_COOKIES    || '';   // cookies my-managment.com
const YAPSON_TOKEN    = process.env.YAPSON_TOKEN     || '';   // accessToken depuis agg.yapson.net > F12 > localStorage.getItem('accessToken')
// ── Mapping réseau → UUID ─────────────────────────────────────
const NET_UUIDS = {
...
// YAPSON_NETWORK : mettre le NOM du reseau choisi
//   - "Orangeint"  → reseau Orangeint
//   - "ORANGE CI"  → reseau Orange CI
// Valeur par defaut : Orangeint
...
const POLL_INTERVAL   = parseInt(process.env.POLL_INTERVAL || '900') * 1000; // défaut 15min
const MAX_SOLDE       = parseInt(process.env.MAX_SOLDE || '0'); // 0 = pas de limite
...
// ── ÉTAPE 1 : Lire le solde yapson ───────────────────────────
async function getSolde() {
  try {
    const res = await fetch('https://connect.yapson.net/api/aggregator/dashboard/', {
      headers: yapsonHeaders(),
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    // Le solde n'est pas dans dashboard, on utilise MAX_SOLDE ou on lit le profil
    // Pour l'instant retourner MAX_SOLDE ou une valeur très grande
    return MAX_SOLDE > 0 ? MAX_SOLDE : 99999999;
  } catch (e) {
    addLog('warn', `Solde non lu: ${e.message} — utilisation MAX_SOLDE`);
    return MAX_SOLDE > 0 ? MAX_SOLDE : 99999999;
  }
}

// ── ÉTAPE 2 : Lire les retraits en attente ───────────────────
async function getWithdrawals(solde) {
  ...
  for (const row of rows) {
    ...
    let phone = '';
    if (row.dopparam && row.dopparam.length > 0) {
      phone = row.dopparam[0].description || '';
    }
    const pm = String(phone).match(/0[0-9]{9}/);
    if (!pm || montant <= 0) continue;
    if (cumul + montant > solde) continue;

    selected.push({
      phone      : pm[0],
      montant,
      id         : row.id,
      confirmData: row.confirm && row.confirm[0] ? row.confirm[0].data : null,
    });
    cumul += montant;
  }
...
// ── ÉTAPE 3 : Créer décaissement yapson ──────────────────────
async function createPayout(item) {
  const uuid = NET_UUIDS[YAPSON_NETWORK] || YAPSON_NETWORK;
...
// ── ÉTAPE 4 : Confirmer retrait my-managment ─────────────────
async function confirmWithdrawal(item) {
  if (!item.confirmData) {
    return { ok: false, err: 'Pas de confirmData' };
  }
  // Construire un FormData simulé via URLSearchParams
  ...
  // Si FormData échoue, essayer JSON direct (ancien endpoint)
  const res2 = await fetch('https://my-managment.com/admin/banktransfer/approvemoney', {
    method : 'POST',
    headers: mgmtHeaders(),
    body   : JSON.stringify(item.confirmData),
  });
...
async function runCycle() {
  ...
  try {
    ...
    state.solde = solde;
    addLog('info', `Solde disponible: ${solde.toLocaleString()} FCFA`);

    // 2. Retraits
    const withdrawals = await getWithdrawals(solde);
    if (withdrawals.length === 0) {
      addLog('info', 'Aucun retrait compatible — prochain cycle dans ' + (POLL_INTERVAL/60000).toFixed(0) + 'min');
      return;
    }
    ...
    for (const item of withdrawals) {
      ...
      if (result.ok) {
        addLog('ok', `✔ Décaissement OK: ${item.phone} → ${item.montant.toLocaleString()} FCFA`);
        paid.push(item);
        state.totalOk++;
        state.totalFCFA += item.montant;
      } else {
        addLog('err', `✘ Décaissement FAIL: ${item.phone} — ${result.err}`);
        state.totalErr++;
      }
      await new Promise(r => setTimeout(r, 800));
    }

    // 4. Confirmations (seulement les décaissements réussis)
    if (paid.length > 0) {
      ...
      for (const item of paid) {
        ...
        if (result.ok) {
          ...
          addLog('warn', `⚠ Confirmation manuelle requise: ${item.phone} — ${result.err || ''}`);
        }
        await new Promise(r => setTimeout(r, 700));
      }
    }

    addLog('ok', `=== Cycle ${state.cycles} terminé: ${paid.length} opération(s) ===`);
  } catch (e) {
    addLog('err', `Erreur cycle: ${e.message}`);
    state.status = 'error';
  } finally {
    state.running = false;
...
function startPolling() {
  if (pollTimer) return;
  addLog('info', `Polling démarré — intervalle: ${POLL_INTERVAL/60000}min`);
  runCycle();
  pollTimer = setInterval(runCycle, POLL_INTERVAL);
}
...
app.get('/', (req, res) => {
  const configured = !!(MGMT_COOKIES && YAPSON_TOKEN);
...
<title>YapsonBot7 — Retraits</title>
...
.lbl{font-size:9px;color:#64748b;margin-top:3px}
...
.badge-warn{background:rgba(251,146,60,.2);color:#fb923c}
...
.ok .le > span:last-child{color:#4ade80}
.err .le > span:last-child{color:#f87171}
.warn .le > span:last-child{color:#fb923c}
.info .le > span:last-child{color:#60a5fa}
...
<div class="app">
  <div class="hdr">
    ...
    <div>
      <h1>YapsonBot7</h1>
      <div class="sub">Automatisation des retraits (Withdrawals)</div>
    </div>
  </div>
  ...
  <div class="grid">
    <div class="card">
      <h3>Statut</h3>
      <span class="badge ${state.status === 'idle' ? 'badge-idle' : state.status === 'running' ? 'badge-run' : state.status === 'error' ? 'badge-err' : 'badge-ok'}">
        ${state.status === 'idle' ? '⏸ En attente' : state.status === 'running' ? '▶ En cours' : state.status === 'error' ? '✘ Erreur' : state.status}
      </span>
      <div class="lbl" style="margin-top:8px">Cycles: ${state.cycles}</div>
      <div class="lbl">Dernier: ${state.lastCycle ? state.lastCycle.replace('T',' ').substring(0,19) : '—'}</div>
    </div>
    <div class="card">
      <h3>Configuration</h3>
      <div class="cfgrow"><div class="dot ${MGMT_COOKIES ? 'ok' : ''}"></div>Cookies my-managment ${MGMT_COOKIES ? '✓' : '✗ manquant'}</div>
      <div class="cfgrow"><div class="dot ${YAPSON_TOKEN ? 'ok' : ''}"></div>Token yapson (agg.yapson.net) ${YAPSON_TOKEN ? '✓' : '✗ manquant'}</div>
      <div class="cfgrow" style="color:#64748b">Réseau: ${YAPSON_NETWORK_NAME} (${YAPSON_NETWORK ? YAPSON_NETWORK.substring(0,8)+'...' : 'inconnu'})</div>
      <div class="cfgrow" style="color:#64748b">Intervalle: ${intervalMin} min</div>
    </div>
    <div class="card">
      <h3>Décaissements OK</h3>
      <div class="stat" style="color:#4ade80">${state.totalOk}</div>
      <div class="lbl">${state.totalFCFA.toLocaleString('fr-FR')} FCFA envoyés</div>
    </div>
    <div class="card">
      <h3>Échecs</h3>
      <div class="stat" style="color:#f87171">${state.totalErr}</div>
      <div class="lbl">Solde actuel: ${state.solde.toLocaleString('fr-FR')} FCFA</div>
    </div>
  </div>

  <div class="actions">
    <a class="btn btn-g" href="/run">▶ Lancer un cycle maintenant</a>
    <a class="btn btn-b" href="/">↻ Actualiser</a>
    <a class="btn btn-r" href="/stop">⏹ Arrêter le polling</a>
    <a class="btn btn-g" href="/start">▶ Démarrer le polling</a>
  </div>

  <div class="card" style="margin-bottom:12px">
    <h3>Journal (${state.log.length} entrées)</h3>
    <div class="log" style="margin-top:8px">${logHtml || '<div class="le info"><span class="lt">—</span><span>Aucun log</span></div>'}</div>
  </div>

  <div style="font-size:9px;color:#64748b;text-align:center;padding:8px">
    YapsonBot7 • Auto-refresh 30s • <a href="/health" style="color:#64748b">health</a>
  </div>
</div>
</body>
</html>`);
...
app.get('/run', async (req, res) => {
  if (state.running) return res.redirect('/');
  runCycle().catch(e => addLog('err', e.message));
  res.redirect('/');
});
...
app.get('/health', (req, res) => {
  res.json({
    ...
    totalErr : state.totalErr,
    totalFCFA: state.totalFCFA,
    running  : state.running,
    configured: !!(MGMT_COOKIES && YAPSON_TOKEN),
  });
});

// ── Endpoint pour injecter les cookies via POST ───────────────
app.post('/cookies', (req, res) => {
  const { cookies, token, network, interval, maxSolde } = req.body || {};
  // Note: en prod les vars d'env sont définies dans Railway
  // Cet endpoint est juste pour le dashboard d'affichage
  res.json({ ok: true, msg: 'Définir MGMT_COOKIES et YAPSON_TOKEN dans les variables Railway' });
});

// ── Démarrage ─────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`YapsonBot7 démarré sur port ${PORT}`);
  addLog('info', `YapsonBot7 démarré — port ${PORT}`);
  addLog('info', `Réseau: ${YAPSON_NETWORK_NAME} → ${YAPSON_NETWORK}`);
  addLog('info', `Intervalle: ${POLL_INTERVAL/60000}min`);
  addLog('info', `Cookies: ${MGMT_COOKIES ? 'OK (' + MGMT_COOKIES.length + ' chars)' : 'MANQUANT'}`);
  addLog('info', `Token: ${YAPSON_TOKEN ? 'OK' : 'MANQUANT'}`);

  if (MGMT_COOKIES && YAPSON_TOKEN) {
    startPolling();
  } else {
    addLog('warn', 'Variables manquantes — définir MGMT_COOKIES et YAPSON_TOKEN dans Railway');
  }
});
